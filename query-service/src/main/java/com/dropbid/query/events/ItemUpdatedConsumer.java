...
@Component
public class ItemUpdatedConsumer {

    private static final Logger log = LoggerFactory.getLogger(ItemUpdatedConsumer.class);

    private static final String STREAM        = "item:updated";
    private static final String GROUP         = "query-service";
    private static final String CONSUMER_NAME = "query-item-consumer-1";
    ...
    void start() {
        ensureConsumerGroup();
        Thread.ofVirtual().name("query-item-consumer").start(this::consumeLoop);
    }

    private void consumeLoop() {
        log.info("Query item consumer started on stream={} group={}", STREAM, GROUP);
        while (!Thread.currentThread().isInterrupted()) {
            try {
                ...
                log.error("Error in query item consumer: {}", e.getMessage(), e);
    ...
    private void processRecord(MapRecord<String, Object, Object> record) {
        try {
            ...
            log.error("Failed to process item:updated record {}: {}", record.getId(), e.getMessage(), e);
        }
    }
